...
export class UsersService {
  ...
  async findOne(id: string): Promise<User | null> {
    try {
      return await this.userRepo.findOne({where: { id },relations: ['orders', 'payments'],
      });
    } catch (err) {
  ...
  async findWithRelations(): Promise<User[]> {
    try {
      return await this.userRepo.find({
        relations: ['orders', 'payments'],
      });
    } catch (err) {
      console.error('Error retrieving users with relations:', err);
      return [];
    }
  }
}
